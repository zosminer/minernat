...
ETH矿池代理中转程序`zosminer`的客户端，用于安装在矿机本地局域网，为所有矿机提供统一入口，上级对接`zosminer`的`ws`协议端口，建立加密伪装隧道。
采用Golang语言开发，性能稳定优异。支持安装为系统服务，开机自启动，支持进程守护运行，程序自动调整连接数限制。Telegram交流群 [点击加入](https://t.me/zosminer_group) 。
...
1. [linux 点击下载 linux-minernat.tar.gz](https://github.com/zosminer/minernat/raw/main/releases/linux-minernat.tar.gz) 。
...
5. 修改配置文件`/etc/minernat/minernat.toml` ，配置`zosminer`端口，
假设你`zosminer`服务器IP是`122.123.0.1`,代理端口是`8888`，协议tls，那么配置文件里面的`zosminer`的值改成`tls://122.123.0.1:8888`。
...
1. [windows 点击下载 windows-minernat.tar.gz](https://github.com/zosminer/minernat/raw/main/releases/windows-minernat.tar.gz) 。
...
6. `cd minernat`
7. `.\minernat.exe init`
8. 修改配置文件`d:\minernat\minernat.toml` ，配置`zosminer`端口，
   假设你`zosminer`服务器IP是`122.123.0.1`,`ws`端口是`8888`，那么配置文件里面的`zosminer`的值改成`ws://122.123.0.1:8888`。
